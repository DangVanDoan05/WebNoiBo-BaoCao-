...
class GeoLocation {
	...
	*/
	...
	function __construct() {
		...
		// TODO: check if the user has set a cookie for the country in the browser
		// if(!$this->cookie()) {
		// 	$this->locate();
		// }
	}
	...
	function cookie() {
		$pbb_country = isset( $_COOKIE['pbb_country'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['pbb_country'] ) ) : '';
		$pbb_continent = isset( $_COOKIE['pbb_continent'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['pbb_continent'] ) ) : '';

		// check if the user has set a cookie for the country
		if ( ! empty( $pbb_country ) ) {
			$user_info = explode( ', ', $pbb_country );

			$this->city	= trim($user_info[0]);
			$this->countryCode	= trim($user_info[1]);
			$this->continentCode	= trim($pbb_continent);
